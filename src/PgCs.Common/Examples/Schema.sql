...
COMMENT ON COLUMN users.preferences IS 'JSON объект с пользовательскими настройками';
...
COMMENT ON COLUMN orders.status_history IS 'История изменений статуса заказа в формате [{status, timestamp, user_id}]';
...
COMMENT ON VIEW active_users_with_orders IS 'Активные пользователи с агрегированной информацией о заказах';
...
COMMENT ON MATERIALIZED VIEW category_statistics IS 'Статистика продаж по категориям (требует REFRESH)';
...
COMMENT ON FUNCTION get_category_path IS 'Возвращает полный путь категории от корня';
...
COMMENT ON TABLE audit_logs IS 'Партиционированная таблица аудита действий пользователей';
...
INSERT INTO users (username, email, full_name, status, preferences, phone_numbers, tags, balance, is_verified, is_premium, date_of_birth) VALUES
('john_doe', 'john@example.com', 'John Doe', 'active', '{"theme": "dark", "language": "en"}', ARRAY['+1234567890', '+1987654321'], ARRAY['vip', 'early_adopter'], 1500.50, TRUE, TRUE, '1990-05-15'),
('jane_smith', 'jane@example.com', 'Jane Smith', 'active', '{"theme": "light", "notifications": true}', ARRAY['+1555666777'], ARRAY['premium'], 500.00, TRUE, FALSE, '1985-08-22'),
('bob_wilson', 'bob@example.com', 'Bob Wilson', 'inactive', '{}', NULL, ARRAY['standard'], 0.00, FALSE, FALSE, '2000-12-01');
...
-- Вставка заказов
INSERT INTO orders (user_id, order_number, status, subtotal, tax, shipping_cost, total, shipping_address, billing_address, payment_method) VALUES
(1, 'ORD-2024-001', 'delivered', 999.99, 80.00, 15.00, 1094.99, 
...
INSERT INTO order_items (order_id, category_id, product_name, sku, quantity, unit_price, total_price, attributes) VALUES
(1, 4, 'MacBook Pro 14"', 'MBP-14-2024', 1, 999.99, 999.99, '{"color": "Space Gray", "ram": "16GB", "storage": "512GB"}'),
(2, 5, 'iPhone 15 Pro', 'IP15P-128', 1, 299.99, 299.99, '{"color": "Titanium", "storage": "128GB"}');