...
SELECT
    u.id, -- comment: Идентификатор пользователя; type: BIGINT; rename: UserId
    ...
    ARRAY_AGG(DISTINCT o.status ORDER BY o.status) FILTER (WHERE o.status IS NOT NULL) AS order_statuses, -- comment: Статусы заказов; type: order_status[]; rename: OrderStatuses
    COUNT(DISTINCT o.id) FILTER (WHERE o.status = 'pending') AS pending_orders, -- comment: Заказов в ожидании; type: BIGINT; rename: PendingOrders
    COUNT(DISTINCT o.id) FILTER (WHERE o.status = 'delivered') AS completed_orders -- comment: Выполненных заказов; type: BIGINT; rename: CompletedOrders
FROM users u
...
SELECT
    c.id, -- comment: Идентификатор категории; type: INTEGER; rename: CategoryId
...
COMMENT ON INDEX idx_category_statistics_revenue IS 'Индекс для сортировки по выручке';
...
CREATE OR REPLACE FUNCTION get_category_path(category_id INTEGER) -- comment: ID категории; type: INTEGER; rename: CategoryId
RETURNS TEXT AS $$
DECLARE
result TEXT := '';
...
CREATE OR REPLACE FUNCTION get_child_categories(parent_category_id INTEGER) -- comment: ID родительской категории; type: INTEGER; rename: ParentCategoryId
